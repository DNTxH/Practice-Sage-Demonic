...
class FireworksRocket extends Entity
{
	...
	public function __construct(Location $location, Fireworks $fireworks, ?int $lifeTime = null)
	{
		...
		$location->getWorld()->broadcastPacketToViewers($this->location, LevelSoundEventPacket::create(56, $this->location->asVector3(),-1,self::getNetworkTypeId(),false,false));
	}
		...
		foreach ($explosionsTag->getValue() as $info) {
			if ($info instanceof CompoundTag) {
				if ($info->getByte("FireworkType", 0) === Fireworks::TYPE_HUGE_SPHERE) {
					$this->getWorld()->broadcastPacketToViewers($this->location, LevelSoundEventPacket::create(58, $this->location->asVector3(),-1,self::getNetworkTypeId(),false,false));
				} else {
					$this->getWorld()->broadcastPacketToViewers($this->location, LevelSoundEventPacket::create(57, $this->location->asVector3(),-1,self::getNetworkTypeId(),false,false));
				}

				if ($info->getByte("FireworkFlicker", 0) === 1) {
					$this->getWorld()->broadcastPacketToViewers($this->location, LevelSoundEventPacket::create(59, $this->location->asVector3(),-1,self::getNetworkTypeId(),false,false));
				}
			}
		}
	}
