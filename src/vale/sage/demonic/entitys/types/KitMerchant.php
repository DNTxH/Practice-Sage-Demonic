...
class KitMerchant extends Human
{
	...
	public function __construct(Location $location, ?Skin $skin = null, ?CompoundTag $nbt = null)
	{
		...
		$this->getInventory()->setItemInHand(ItemFactory::getInstance()->get(ItemIds::ENDER_CHEST));
	...
	public function attack(EntityDamageEvent $source): void
	{
		if ($source instanceof EntityDamageByEntityEvent) {
			...
			if ($damager instanceof Player) {
				$session = Loader::getInstance()->getSessionManager()->getSession($damager);
				$k = new KitsInventory();
				$k->sendMenu($damager);
			}
		}
