...
class LegendaryCrate extends Crate
{
	...
	public function __construct(private Position $position, private Item $key){
		...
		parent::__construct(self::ELITE, $position, $key, [
			...
			new Reward(($item = Rewards::get(Rewards::RANDOM_MONEY_GENERATOR,1)), function(Player $player)use($item): void{
				$drops = $player->getInventory()->addItem($item);
				foreach($drops as $drop) $player->getWorld()->dropItem($player->getPosition(), $drop);
			}, 1),

			new Reward(($item = Rewards::get(Rewards::BLAZE_GRAB_BAG,rand(1,2))), function(Player $player)use($item): void{
				$drops = $player->getInventory()->addItem($item);
				foreach($drops as $drop) $player->getWorld()->dropItem($player->getPosition(), $drop);
			}, 56),
	...
	public function broadcastParticle(Player $player): void{
		$particle = new BlockBreakParticle(VanillaBlocks::CONCRETE()->setColor(DyeColor::ORANGE()));
