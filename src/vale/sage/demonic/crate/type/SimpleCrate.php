...
class SimpleCrate extends Crate
{
	...
	public function __construct(private Position $position, private Item $key){
		...
		parent::__construct(self::ELITE, $position, $key, [
			...
			new Reward(($item = $factory->get(ItemIds::GOLD_BLOCK, 0, 12)), function(Player $player)use($item): void{
				$drops = $player->getInventory()->addItem($item);
				foreach($drops as $drop) $player->getWorld()->dropItem($player->getPosition(), $drop);
			}, rand(1,100)),

			new Reward(($item = EnchantFactory::getEnchantDust("simple",true,1)), function(Player $player)use($item): void{
				$drops = $player->getInventory()->addItem($item);
				foreach($drops as $drop) $player->getWorld()->dropItem($player->getPosition(), $drop);
			}, rand(1,100)),

			new Reward(($item = EnchantFactory::getEnchantDust("simple",false,1)), function(Player $player)use($item): void{
				$drops = $player->getInventory()->addItem($item);
				foreach($drops as $drop) $player->getWorld()->dropItem($player->getPosition(), $drop);
			}, rand(1,100)),

			new Reward(($item = EnchantFactory::getEnchantRandomizer("simple")), function(Player $player)use($item): void{
				$drops = $player->getInventory()->addItem($item);
				foreach($drops as $drop) $player->getWorld()->dropItem($player->getPosition(), $drop);
			}, rand(1,100)),
			...
			new Reward(($item = EnchantFactory::giveEnchantUtilities("weaponorb",rand(1,1))), function(Player $player)use($item): void{
				$drops = $player->getInventory()->addItem($item);
				foreach($drops as $drop) $player->getWorld()->dropItem($player->getPosition(), $drop);
			}, rand(1,100)),

			new Reward(($item = EnchantFactory::createSoulGem(null,120)), function(Player $player)use($item): void{
				$drops = $player->getInventory()->addItem($item);
				foreach($drops as $drop) $player->getWorld()->dropItem($player->getPosition(), $drop);
			}, rand(1,100)),
			...
			new Reward(($item = RedeemableAPI::getMCCrate(0)), function(Player $player)use($item): void{
				$drops = $player->getInventory()->addItem($item);
				foreach($drops as $drop) $player->getWorld()->dropItem($player->getPosition(), $drop);
			}, 23),
			...
			new Reward(($item = Rewards::getArmorCrystal(0,1)), function(Player $player)use($item): void{
				$drops = $player->getInventory()->addItem($item);
				foreach($drops as $drop) $player->getWorld()->dropItem($player->getPosition(), $drop);
			}, 10),

			new Reward(($item = Rewards::get(Rewards::ULITMATE_BOOK,1)), function(Player $player)use($item): void{
				$drops = $player->getInventory()->addItem($item);
				foreach($drops as $drop) $player->getWorld()->dropItem($player->getPosition(), $drop);
			}, 1),

			new Reward(($item = Rewards::get(Rewards::LEGENDARY_BOOK,rand(1,2))), function(Player $player)use($item): void{
				$drops = $player->getInventory()->addItem($item);
				foreach($drops as $drop) $player->getWorld()->dropItem($player->getPosition(), $drop);
			}, 56),
			new Reward(($item = Rewards::get(Rewards::DEMONIC_BOOK,rand(1,2))), function(Player $player)use($item): void{
				$drops = $player->getInventory()->addItem($item);
				foreach($drops as $drop) $player->getWorld()->dropItem($player->getPosition(), $drop);
			}, 56),
			new Reward(($item = $factory->get(ItemIds::DIAMOND_BLOCK, 0, 124)), function(Player $player)use($item): void{
				$drops = $player->getInventory()->addItem($item);
				foreach($drops as $drop) $player->getWorld()->dropItem($player->getPosition(), $drop);
			}, 11),
		]);
	...
	public function broadcastParticle(Player $player): void{
		$particle = new BlockBreakParticle(VanillaBlocks::STONE());
